...
-- toColor3 has been moved to Colour.hs to prevent circular dependencies
...
boundingBox vs = ((x-dw,y-dh),(x'+dw,y'+dh))
  where dh = 0.02 * abs (y'-y)
        dw = 0.02 * abs (x'-x)
        ((x,y),(x',y')) = foldl' f ((infinity, infinity), (-infinity, -infinity)) vs
